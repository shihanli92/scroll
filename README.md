...
Each card's control column holds *what to plot*; **how it looks** lives in the
plot's **Style sheet**, opened from the paintbrush button in its toolbar. The sheet
updates the plot live and holds:

- **Plot**: the panel's own look controls (palette, point size, opacity, aspect ratio, …).
- **Titles & labels**: title, subtitle, caption, axis labels, legend title.
- **Theme**: text sizes and font, legend position, axes, gridlines, border, strips and
...
(vector, for figures/slides) from its toolbar; the DE table also exports to **CSV**.
Custom panels get the sheet too: wrap a look-only control in `scroll_style_input()`
to place it there.
