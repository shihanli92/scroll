...
vignette: >
  ...
  %\VignetteIndexEntry{Custom panel: a neighbourhood explorer}
...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  eval = FALSE          # narrated: needs a Seurat object, a built project,
)                       # or a running app, so nothing runs at build time
```
...
-   A built `pbmc3k` project --- the `pbmc3k/` directory. The baked
    graph goes inside it.
...
between cells. To get these we need to rerun `FindNeighbors()` with
`return.neighbor = TRUE`; otherwise the call only stores a binary
...
function that renders the ggplot. The `register_plot_panel()` function
then attaches these to the app. For this article, we will slowly build
up the nearest-neighbour panel with increasing complexity.
...
The plot function receives `cells`, the `input` values, and `data` ---
the last used to find the baked graph via `data$dir` --- and returns a
ggplot. Note that while we use a Parquet file to store the data,
realistically any readable data format works for custom panels, with the
caveat that memory constraints need to be considered.
...
Call `register_plot_panel()` to add the panel to the app. Adding
`after = "featureplot"` places the panel after FeaturePlot, and
`compute = TRUE` adds a Compute button that stops the graph from
updating every time an option is changed.
...
input into a control. Here we add two `colourpicker::colourInput`s, one
...
> This registers in memory only. `register_plot_panel()` just adds the
> panel to the current session's registry. For a reproducible,
> deployable app, put it in its own file.
...
Create the folder --- scroll won't make it for you --- and drop the
panel code into it:
...
Source order matters: `register_plot_panel()` must run *before*
`scroll_app()` builds the UI, which is why the `source()` comes first.
The `scroll_reset_panels()` at the top clears any prior registration so
re-running `app.R` doesn't add the panel twice. Launch it from the app
folder:
